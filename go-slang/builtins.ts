...
const builtin_implementation: Record<string, BuiltinFunction> = {
  ...
  is_pair: (machine, heap, _) =>
    heap.is_Pair(machine.OS.pop()!) ? heap.values.True : heap.values.False,
  head: (machine, heap, _) => heap.get_child(machine.OS.pop()!, 0),
  tail: (machine, heap, _) => heap.get_child(machine.OS.pop()!, 1),
  is_null: (machine, heap, _) =>
    heap.is_Null(machine.OS.pop()!) ? heap.values.True : heap.values.False,
...
export const builtins: Record<
  string,
  { tag: string; id: number; arity: number }
> = {};
export const added_builtins: Record<
  string,
  { tag: string; id: number; arity: number }
> = {};
export const builtin_array: BuiltinFunction[] = [];
export const builtin_id_to_arity: Record<number, number> = {};
const added_builtins_implementation = {
  ...array_builtins,
  ...waitGroup_builtins,
  ...mutex_builtins,
};
